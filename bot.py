...
async def tarot(update: Update, context: CallbackContext) -> None:
    ...
    caption = f"Ваша карта на сегодня:\n\n*{card['name']}*\n\n{card['meaning']}"
    try:
...
async def name_input(update: Update, context: CallbackContext) -> None:
    ...
    card = random.choice(tarot_cards)
    caption = f"{name}, ваша карта на сегодня: *{card['name']}*\n\n{card['meaning']}"
    try:
        with open(card['image_path'], 'rb') as image_file:
            await context.bot.send_photo(chat_id=update.effective_chat.id, photo=image_file, caption=caption, parse_mode=ParseMode.MARKDOWN)
    except FileNotFoundError:
        await update.message.reply_text(f"{name}, изображение карты {card['name']} не найдено. Пожалуйста, проверьте путь к файлу.")

    prompt = f"""Напиши толкование карты Таро *{card['name']}* для {gender} по имени {name} в следующем стиле:
        *{card['name']}* – [Краткое описание карты, её основное значение]
    ...
    model = genai.GenerativeModel('gemini-2.0-flash-001')  
    response = model.generate_content(prompt)   
    ai_insight = response.text.lstrip("#").strip()
